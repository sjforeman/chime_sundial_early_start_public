...
def interactive_resolution():
    """
    Interactive diffraction-limited resolution activity.
    Values can be changed using either sliders or input boxes.
    """
    ...
    D_slider = w.FloatLogSlider(
        value=2.4, base=10, min=-2.3, max=6,
        step=0.02,
        description="D [m]",
        readout=False,
    ...
    # Connect slider and box
    w.jslink((D_slider, "value"), (D_box, "value"))

    # Wavelength dropdown
    wl_dd = w.Dropdown(
        options=WAVELENGTH_PRESETS,
        value=550e-9,
        description="λ"
    )
    ...
    def _update(D, wavelength, sep_over_theta):
        theta = show_two_sources(
            D=D,
            wavelength=wavelength,
            sep_over_theta=sep_over_theta
        )

        #out_lbl.value = (
        #    f"<b>θ = 1.22 λ/D = {_fmt_angle(theta)}</b>"
        #)
        ...
        out_lbl.value = (
            ...
            f'margin-top: 8px;'
            f'">'
            f'θ = 1.22 λ/D = {_fmt_angle(theta)}'
            f'</div>'
        )

    # Layout
    D_row = w.HBox([D_slider, D_box])
    sep_row = w.HBox([sep_slider, sep_box])
